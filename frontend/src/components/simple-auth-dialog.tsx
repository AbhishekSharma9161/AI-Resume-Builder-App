...
export function SimpleAuthDialog({ children }: SimpleAuthDialogProps) {
  ...
  const handleGoogleSignIn = () => {
    ...
    // Demo Google accounts to choose from
    const demoAccounts = [
      {
        id: "1",
        name: "John Smith",
    ...
    setTimeout(() => {
      const accountList = demoAccounts.map((acc, idx) => 
        `${idx + 1}. ${acc.name} (${acc.email})`
      ).join('\n');
      
      const choice = prompt(
        `🔐 Google Account Selection (Demo Mode)\n\n` +
      ...
      if (choice && ['1', '2', '3'].includes(choice)) {
        const selectedAccount = demoAccounts[parseInt(choice) - 1];
        signIn(selectedAccount);
        setIsOpen(false);
        setIsLoading(false);
        
        // Show success message
        alert(`✅ Successfully signed in as ${selectedAccount.name}!`);
